...
export function readConfig(cwd: string = process.cwd()): SuedeConfig | null {
  ...
  try {
    ...
    if (
      ...
      "apiUrl" in parsed &&
      typeof (parsed as Record<string, unknown>).workspaceKey === "string" &&
      typeof (parsed as Record<string, unknown>).apiUrl === "string"
    ) {
      return parsed as SuedeConfig;
    }
    return null;
  } catch {
    return null;
...
export function writeConfig(cwd: string, config: SuedeConfig): void {
...
export function buildInitFiles(apiUrl: string): InitFile[] {
...
async function runLogin(key: string, cwd: string): Promise<void> {
  if (!key) {
 ...
 */
async function loadManifestFromAgentFile(agentPath: string): Promise<Record<string, unknown>> {
  ...
  try {
    const mod = await import(agentPath) as { default?: { name?: string; description?: string; triggers?: unknown[] } };
...
export async function runPush(config: SuedeConfig, cwd: string): Promise<PushOutput> {
  ...
  if (!result.slug) throw new Error("Push failed: server did not return a slug");
...
export async function runPull(slug: string, config: SuedeConfig, cwd: string): Promise<void> {
  ...
  const result = (await res.json()) as { manifest?: Record<string, unknown> };
  const manifest = result.manifest;
  if (!manifest) throw new Error("Pull failed: server did not return a manifest");
...
function versionClient(config: SuedeConfig) {
...
function shortVersionHash(hash: string): string {
  return hash.length > 12 ? hash.slice(0, 12) : hash;
}

function formatVersionSummary(version: FlowVersionSummary): string {
...
export async function runVersionsList(
  flowId: string,
  config: SuedeConfig,
): Promise<string> {
...
function formatDependency(dependency: FlowVersionRecord["dependencies"][number]): string {
...
export async function runVersionInspect(
  flowId: string,
  versionId: string,
  config: SuedeConfig,
): Promise<string> {
...
function safeOpaquePathSegment(value: string): string {
...
function assertSafeOutputTarget(cwd: string, target: string): void {
...
function assertTrustedOutputDirectories(
  cwd: string,
  ...
  const directories = [
    root,
    ...relative.split(path.sep).filter(Boolean).map((_, index, segments) =>
...
function assertExistingTargetIsSafe(target: string, force: boolean): void {
  try {
...
function ensureVersionStagingDirectory(cwd: string): string {
  const root = path.resolve(cwd);
...
function assertSameFilesystem(staging: string, targetDirectory: string): void {
...
function writeAtomic(cwd: string, target: string, content: string, force: boolean): void {
  ...
  const temporary = path.join(
    staging,
    `.version.${process.pid}.${atomicWriteSequence}.tmp`,
  );
  try {
...
export async function runVersionPull(
  flowId: string,
  versionId: string,
  config: SuedeConfig,
  cwd: string,
...
export async function runVersionExport(
  ...
  const target = options.out === undefined
    ? path.join(
        ...
        ".suede",
        "versions",
        `${safeOpaquePathSegment(versionId)}.suede-version.json`,
      )
    : path.resolve(cwd, options.out);
...
function parseVersionWriteOptions(args: readonly string[]): VersionWriteOptions {
  ...
  switch (subcommand) {
    ...
    case "pull": {
      ...
      const target = await runVersionPull(
        ...
        config,
        cwd,
        parseVersionWriteOptions(flags),
      );
    ...
    case "export": {
      ...
      const target = await runVersionExport(
        flowId,
        versionId,
        config,
        cwd,
    ...
    default:
      throw new Error(
        "Usage: suede versions <list|pull|inspect|export> <flowId> [versionId]",
      );
  }
}
...
function generateAgentTs(manifest: Record<string, unknown>): string {
  ...
  const triggerExprs = triggers.map((t: unknown) => {
    if (typeof t !== "object" || t === null) return `{ kind: "manual" }`;
    const trig = t as Record<string, unknown>;
    if (trig.kind === "schedule") return `schedule(${JSON.stringify(trig.cron)})`;
    if (trig.kind === "paidCall") return `paidCall(${Number(trig.priceUsdc ?? 0)})`;
    if (trig.kind === "webhook") return `{ kind: "webhook" }`;
    return `{ kind: "manual" }`;
  }).join(",\n    ");

  const imports = triggers.some((t: unknown) => (t as Record<string, unknown>)?.kind === "schedule")
    ? `import { defineAgent, schedule, paidCall, suede } from "@suedeai/agents";`
    : `import { defineAgent, paidCall, suede } from "@suedeai/agents";`;

  const stepLines = steps.map((s: unknown) => {
...
// ────────────────────────────────────────────────────────────────────────────
// link — register a relay URL for a published agent
// ────────────────────────────────────────────────────────────────────────────
...
export async function runLink(
  ...
  if (!slug) throw new Error("Usage: suede link <slug> --url <url>");
  if (!url) throw new Error("Usage: suede link <slug> --url <url>");
  ...
  const result = (await res.json()) as { secret?: string; url?: string };
  if (!result.secret) throw new Error("Link failed: server did not return a secret");
...
async function runDev(cwd: string): Promise<void> {
  ...
  try {
    const mod = await import(agentPath) as { default?: import("./types.js").AgentDefinition };
    const agent = mod.default;
    if (!agent) {
...
function runWhoami(cwd: string): void {
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "link": {
      ...
      process.stdout.write(
        `Linked. Suede hosts the paid endpoint; your machine does the work. Keep this secret safe — it's shown once.\n\n` +
