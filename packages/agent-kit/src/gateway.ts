 ...
 */
async function callRun(nodeType: string, config: Record<string, unknown>): Promise<RunResult> {
  ...
  if (response.status === 402) {
    ...
    try {
      const body = await response.json() as { topup?: { topupEndpoint?: string; tiers?: number[] } };
...
async function callLlm(input: LlmInput): Promise<LlmResult> {
  const stub = process.env["SUEDE_GATEWAY_STUB"];
  if (stub === "1") {
