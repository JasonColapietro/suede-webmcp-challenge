...
import {
  ...
  CreateResourceSourceRequestSchema,
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const body = CreateResourceSourceRequestSchema.safeParse(read.data);
    if (!body.success) return privateJson({ error: "invalid request" }, 400);
    if (body.data.kind === "url") {
    ...
    return privateJson(await service.collectSource(ownerId, resourceId, body.data), 201);
  } catch (error: unknown) {
