/**
 * A2A 1.0 agent card for a Suede agent.
 * GET /api/agents/[agent]/.well-known/agent-card — advertises the endpoint,
 * provider, current call capabilities, and payment state.
 */
...
export async function GET(_req: Request, { params }: RouteContext): Promise<NextResponse> {
  try {
    ...
    // Per-agent identity, not boilerplate: a crawler comparing two cards must
    // see two different agents. Name is the flow's human name, description is
    // the creator's own pitch (node-chain summary as the fallback), the url is
    // absolute, and the input schema is derived from the public graph.
    ...
    return NextResponse.json(buildSuedeAgentCard({
      ...
      slug: agent.slug,
      description: service.kind === "resource" ? service.description : contract?.description ?? description,
    ...
    console.error("agents .well-known agent-card route failed", error);
