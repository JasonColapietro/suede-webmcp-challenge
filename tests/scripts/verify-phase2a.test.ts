...
const libraryFile = resolve(process.cwd(), "scripts/verify-phase-2a-lib.mjs");
const wrapperFile = resolve(process.cwd(), "scripts/verify-phase-2a.mjs");

async function loadLibrary() {
  const specifier = `../../scripts/verify-phase-2a-lib.mjs?test=${Date.now()}-${Math.random()}`;
  return import(/* @vite-ignore */ specifier) as Promise<{
    PHASE2A_STEPS: ReadonlyArray<
      | { readonly kind: "command"; readonly command: string; readonly args: readonly string[] }
      | { readonly kind: "remove"; readonly path: string }
    >;
    runPhase2aVerification(options?: Record<string, unknown>): void;
  }>;
}

describe("Phase 2A release verifier", () => {
  ...
  it("freezes the focused, full, SDK, cleanup, and Next gates in release order", async () => {
    const { PHASE2A_STEPS } = await loadLibrary();
    const printable = PHASE2A_STEPS.map((step) =>
    ...
    expect(printable.join("\n")).not.toContain("verify:phase2a");
  });

  it("runs through the hardened shared evidence runner with a Phase 2A label", async () => {
    const { runPhase2aVerification } = await loadLibrary();
    const events: string[] = [];
    const output: string[] = [];
    ...
    expect(output.join("")).toContain("Phase 2A verification commit: commit");
    expect(output.join("")).toContain("Phase 2A verification passed");
  });
